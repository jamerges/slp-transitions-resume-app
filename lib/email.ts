...
/**
 * A story pitch from /share-your-story, sent to James's own inbox.
 *
 * replyTo is the submitter, so hitting Reply starts the conversation with them
 * rather than with the sending domain — the whole point of the form is to open
 * that thread. Everything is escaped: this is stranger-supplied text landing in
 * an HTML email.
 */
export async function sendStorySubmission(s: StorySubmission): Promise<void> {
  const to = process.env.OPS_ALERT_EMAIL || "jamoberges@gmail.com";
  const row = (k: string, v?: string) =>
    v
      ? `<tr><td style="padding:6px 14px 6px 0;color:#6B7280;white-space:nowrap;vertical-align:top">${esc(
          k
        )}</td><td style="padding:6px 0;vertical-align:top">${nl2br(esc(v))}</td></tr>`
      : "";
  ...
  await getResend().emails.send({
    ...
    to,
    replyTo: s.email,
    subject: `Story pitch: ${s.name} — ${s.now}`.slice(0, 120),
    html: `<div style="font-family:'DM Sans',Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:#1B1B1E">
      <p style="margin:0 0 14px">New story submission from <strong>${esc(s.name)}</strong>. Reply to this email to reach them directly.</p>
