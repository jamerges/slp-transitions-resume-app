...
def build():
    ...
    for n, (t, d) in enumerate([
        ...
        a(f'<article class="slp-path-card"><span class="slp-path-num">0{n}</span>'
    ...
    for i, (t, c) in enumerate(AFFIRMATIONS, 1):
        a(f'<div><span class="slp-tick">✓</span><h3>{esc(t)}</h3><p>{esc(c)}</p></div>')
    a('</div></div></section>')
    ...
    a('<div class="slp-cards">')
    for c in PATHS:
        a('<article class="slp-card">')
    ...
    for s in STORIES:
        a(f'<a class="slp-story" href="{s["href"]}">'
    ...
    a('<div class="slp-res">')
    for r in RESOURCES:
        a(f'<a href="{r["href"]}"><span class="step">{esc(r["step"])}</span>'
