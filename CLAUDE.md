...
- **Scroll animation layer (2026-08-02):** three coordinated pieces, all vanilla JS inside the generated block — (1) the hero route SVG draws itself through the 3 pathway cards on first view; (2) a fixed left "journey rail" (≥1200px only) where a split-road diamond marker travels with scroll past 5 clickable milestones (data-mile attrs on sections); (3) reveal animations via a scroll-loop sweep, NOT IntersectionObserver — instant jumps (milestone clicks) stranded IO-based reveals invisible. Everything gates on `.js-anim` (added by JS) so no-JS/reduced-motion users get the full static page with zero CLS.
